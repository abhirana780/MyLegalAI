...
# State codes mapping for eCourts API
state_codes = {
    "Andhra Pradesh": "AP",
...
if st.button("🔍 Search Cases", use_container_width=True):
    with st.spinner("Searching for cases..."):
        # Fetch live case data from eCourts API
        from legal_data import fetch_case_status
        
        # Format case number according to eCourts API requirements
        formatted_case_number = case_number.replace("/", "-")
        
        # Get state code for selected state
        state_code = state_codes.get(selected_state)
        
        # Fetch case details
        case_details = fetch_case_status(formatted_case_number, state_code)
        
        if case_details:
            # Convert API response to DataFrame
            case_data = {
                "case_number": case_details.get("cnr_number"),
