...
class EnhancedLegalCaseMatcher:
    ...
    def fetch_live_cases():
        """Fetch live case data from Indian Kanoon and Supreme Court websites"""
        cases = []
        sources = [
        ...
        for url in sources:
            try:
                ...
                case_elements = soup.find_all(['div', 'article'], class_=['case-listing', 'judgment-item'])
                
                for case in case_elements:
                    title = case.find(['h2', 'h3', 'h4'])
                    summary = case.find(['p', 'div'], class_='summary')
                    date = case.find(['span', 'time'], class_='date')
    ...
    def __init__(self):
        ...
        self.tfidf_vectorizer = TfidfVectorizer(
            max_features=5000,  # Reduced feature count
            ngram_range=(1, 2),
            stop_words='english',
            use_idf=True,
            smooth_idf=True,
